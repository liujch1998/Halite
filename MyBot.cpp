...
int main ()
{
	...
	while(true)
	{
		...
		boarders.clear ();
		int myTerritoryArea = 0;
		for (unsigned short a = 0; a < presentMap.height; a ++)
		{
			for (unsigned short b = 0; b < presentMap.width; b ++)
			{
				if (presentMap.getSite ({b, a}, STILL).owner == myID) { myTerritoryArea ++; continue; }
				bool isBoarder = 0;
			...
			for (unsigned short b = 0; b < presentMap.width; b++)
			{
				if (presentMap.getSite ({b, a}).owner == myID)
				{
					...
					for (std::set<hlt::Location>::iterator it = boarders.begin (); it != boarders.end (); it ++)
					{
						...
						float force;
						if (myTerritoryArea >= 8)
							force = (float) presentMap.getSite (*it, STILL).production / (dist * dist);
						else force = (1.0f / (float) presentMap.getSite (*it, STILL).strength) / (dist * dist);
